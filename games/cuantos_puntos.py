...
def _next_question():
    ...
    # Valor inicial del spinner: mitad del valor correcto aprox
    _, _, correct, _ = st.session_state.pg_q
    st.session_state.pg_guess = max(1, correct // 2)

def _submit_guess():
    if st.session_state.pg_round_done or not st.session_state.pg_q:
        return
    guess = st.session_state.pg_guess
    _, _, correct, info = st.session_state.pg_q
...
def render():
    ...
    with col2:
        new_daily = st.toggle("📅 Diario", key="pg_daily_tog",
                              value=st.session_state.pg_daily)
        if new_daily != st.session_state.pg_daily:
            st.session_state.pg_daily = new_daily
            st.rerun()

    if st.session_state.pg_daily and daily_played("pointsguess") and not st.session_state.pg_done:
    ...
    if st.session_state.pg_done:
        ...
        e, t = ("🏆","¡Cronista perfecto!") if pct>=80 else ("🥇","¡Muy buen ojo!") if pct>=50 else ("🥈","Bien, sigue practicando") if pct>=30 else ("🤔","Los puntos de F1 son complicados...")
    ...
    if st.session_state.pg_intentos:
        ...
        for state, val, _ in st.session_state.pg_intentos:
            if state == "exact":
                ...
                chips.append(f"<span style='background:#1565c0;color:white;padding:3px 10px;border-radius:6px;margin:2px'>🔼 {val}</span>")
            else:
                chips.append(f"<span style='background:#b71c1c;color:white;padding:3px 10px;border-radius:6px;margin:2px'>🔽 {val}</span>")
    ...
    if st.session_state.pg_round_done:
        ...
        if st.session_state.pg_intentos:
            ...
            if last[0] == "low":
                st.info(f"🔼 El valor es MÁS que {last[1]}")
            elif last[0] == "high":
                st.warning(f"🔽 El valor es MENOS que {last[1]}")

        intentos_left = MAX_INTENTOS - len(st.session_state.pg_intentos)
        current_val   = st.session_state.pg_guess

        # Mostrar valor actual grande
        st.markdown(
            f"<div style='text-align:center;font-size:36px;font-weight:bold;color:#e10600;"
            f"padding:8px;background:#1a1a1a;border-radius:8px;margin:8px 0'>"
            f"{current_val} pts</div>",
            unsafe_allow_html=True
        )

        # Botones de ajuste en una sola fila
        c1, c2, c3, c4, c5 = st.columns(5)
        with c1:
            if st.button("−10", use_container_width=True):
                st.session_state.pg_guess = max(0, current_val - 10)
                st.rerun()
        with c2:
            if st.button("−1", use_container_width=True):
                st.session_state.pg_guess = max(0, current_val - 1)
                st.rerun()
        with c3:
            if st.button("+1", use_container_width=True):
                st.session_state.pg_guess = min(700, current_val + 1)
                st.rerun()
        with c4:
            if st.button("+10", use_container_width=True):
                st.session_state.pg_guess = min(700, current_val + 10)
                st.rerun()
        with c5:
            if st.button(f"✔ ({intentos_left})", type="primary", use_container_width=True):
                _submit_guess()
                st.rerun()
